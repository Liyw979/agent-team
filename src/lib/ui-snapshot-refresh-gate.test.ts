...
test("较新的 ui snapshot 响应一旦已被接受，较旧响应必须被拒绝", () => {
  ...
  assert.equal(requireTaskAgent(readAcceptedPayload(acceptedNewer.latestAcceptedState), "UnitTest").status, "running");
...
test("ui snapshot 门禁允许首次响应和更大请求号通过，但拒绝相同请求号重复回写", () => {
  ...
  assert.equal(requireTaskAgent(readAcceptedPayload(newerAccepted.latestAcceptedState), "BA").status, "completed");
});
...
test("较新的请求号把任务从 finished 重新带回 running 时，门禁必须接受这次合法 reopen", () => {
  ...
  const acceptedReopenedPayload = readAcceptedPayload(reopenedRunning.latestAcceptedState);
  assert.equal(acceptedReopenedPayload.kind, "task");
  assert.equal(acceptedReopenedPayload.task.task.status, "running");
});
...
test("较小请求号若晚返回且语义上更新，门禁仍必须接受", () => {
  ...
  const acceptedLowerPayload = readAcceptedPayload(acceptedLowerRequest.latestAcceptedState);
  assert.equal(acceptedLowerPayload.kind, "task");
  assert.equal(acceptedLowerPayload.task.messages.length, 3);
});
...
test("较小请求号若首次带回新的 runtime agent，也必须被视为语义前进并接受", () => {
  ...
  assert.equal(acceptedLowerRequest.accepted, true);
  assert.equal(
    requireTaskAgent(readAcceptedPayload(acceptedLowerRequest.latestAcceptedState), "误报论证-2").id === "误报论证-2",
    true,
  );
});
...
test("语义上更新的 snapshot 必须覆盖旧缓存", () => {
  ...
  assert.equal(acceptedPayload.kind, "task");
...
test("较小请求号若首次带回新的 runtime agent，查询缓存也必须采用新 snapshot", () => {
  ...
  const acceptedPayload = resolveUiSnapshotQueryData(previousPayload, nextPayload);
  assert.equal(acceptedPayload, nextPayload);
  assert.equal(requireTaskAgent(acceptedPayload, "误报论证-2").id, "误报论证-2");
});
