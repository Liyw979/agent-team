...
test("renderMarkdownToStaticHtml 会恢复列表 marker，避免全局 reset 后只剩缩进空白", () => {
  const html = renderMarkdownToStaticHtml("- 第一项\n- 第二项");

  assert.match(html, /\.chat-markdown ul \{[^}]*list-style-type: disc;/);
  assert.match(html, /\.chat-markdown ol \{[^}]*list-style-type: decimal;/);
});
