...
test("在下游配置中把某个下游勾选为 spawn 后，会自动把该下游及其后续可达 Agent 组成同一个动态团队", () => {
  const topology: TopologyRecord = {
    ...
    edges: [
      { source: "Build", target: "正方", triggerOn: "association", messageMode: "last" },
      { source: "正方", target: "反方", triggerOn: "needs_revision", messageMode: "last" },
      { source: "反方", target: "Summary", triggerOn: "approved", messageMode: "last" },
    ],
    spawnRules: [],
  };
...
test("启用 spawn 时，会清掉同一下游上的其它触发类型，保证四种模式完全互斥", () => {
  const topology: TopologyRecord = {
    ...
    edges: [
      { source: "Build", target: "正方", triggerOn: "association", messageMode: "last" },
      { source: "Build", target: "正方", triggerOn: "approved", messageMode: "last" },
      { source: "Build", target: "正方", triggerOn: "needs_revision", messageMode: "last" },
      { source: "正方", target: "反方", triggerOn: "needs_revision", messageMode: "last" },
      { source: "反方", target: "Summary", triggerOn: "approved", messageMode: "last" },
    ],
    spawnRules: [],
    ...
    spawnRules: [
      {
        ...
        edges: [
          { sourceRole: "entry", targetRole: "反方", triggerOn: "association", messageMode: "last" },
          { sourceRole: "反方", targetRole: "Summary", triggerOn: "association", messageMode: "last" },
        ],
...
test("切换到审视不通过时，会关闭 spawn 并保留一条可调度的 needs_revision 入口边", () => {
  const topology: TopologyRecord = {
...
test("当前下游模式会在 spawn、传递、审视通过、审视不通过 四种触发里返回唯一结果", () => {
  ...
  const associationTopology: TopologyRecord = {
    ...
    edges: [{ source: "Build", target: "正方", triggerOn: "association", messageMode: "last" }],
    spawnRules: [],
  };
  assert.equal(
    getDownstreamMode({
  ...
  const passTopology: TopologyRecord = {
    ...
    edges: [{ source: "Build", target: "正方", triggerOn: "approved", messageMode: "last" }],
    spawnRules: [],
  };
  assert.equal(
    getDownstreamMode({
  ...
  const failTopology: TopologyRecord = {
    ...
    edges: [{ source: "Build", target: "正方", triggerOn: "needs_revision", messageMode: "last" }],
    spawnRules: [],
  };
  assert.equal(
    getDownstreamMode({
