...
function createWorkspaceAndTask(): {
  ...
  const workspace: WorkspaceSnapshot = {
    ...
    agents: [
      ...
      { id: "误报论证", prompt: "挑战", isWritable: false },
  ...
  const task: TaskSnapshot = {
    ...
    messages: [
      ...
      },
      {
        ...
        sender: "线索发现",
        content: "@误报论证-1",
        timestamp: toUtcIsoTimestamp("2026-04-30T10:00:00.500Z"),
        kind: "agent-dispatch", targetAgentIds: ["误报论证-1"],
        targetRunCounts: [1],
        dispatchDisplayContent: "@误报论证-1",
        senderDisplayName: "线索发现",
      },
    ],
    topology,
...
test("ChatWindow 会在 agent 结束后移除动态面板，并由最终消息接管原位置", async () => {
  ...
  const settledTask: TaskSnapshot = {
    ...
    messages: [
      ...
      },
      {
        ...
        kind: "agent-dispatch", targetAgentIds: ["漏洞论证-1"],
        targetRunCounts: [1],
        dispatchDisplayContent: "当前证据不足以证明这里一定能越界写入。\n\n@漏洞论证-1",
        senderDisplayName: "误报论证-1",
      },
    ],
  };
  ...
  try {
    await act(async () => {
      root.render(
        <ChatWindow
          workspace={workspace}
          task={runningTask}
          availableAgents={["线索发现", "误报论证", "漏洞论证", "讨论总结"]}
          taskLogFilePath=""
          taskUrl=""
    ...
    const senderLabels = Array.from(container.querySelectorAll("span"))
      ...
      .filter(Boolean);
    assert.equal(
      senderLabels.filter((label) => label === "误报论证-1").length,
      1,
    );
    assert.equal(
