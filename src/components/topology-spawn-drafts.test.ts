...
test("getTopologyDisplayNodeIds 会隐藏静态 spawn 节点，只展示真正的 agent 节点", () => {
  const topology: TopologyRecord = {
    ...
    nodeRecords: [
      ...
      { id: "疑点辩论工厂", kind: "spawn", templateName: "漏洞论证模板", spawnRuleId: "spawn-rule:漏洞团队" },
    ],
    edges: [],
    spawnRules: [],
  };

  assert.deepEqual(getTopologyDisplayNodeIds(topology, []), ["线索发现"]);
});
...
test("getTopologyDisplayNodeIds 会用 runtime 实例替换已展开的静态模板节点", () => {
  ...
  assert.deepEqual(
    getTopologyDisplayNodeIds(topology, ["线索发现", "漏洞挑战", "漏洞论证", "漏洞论证-1", "讨论总结", "讨论总结-1"]),
...
test("upsertDebateSpawnDraft 会生成 GUI 需要保存的 spawn 节点、spawnRule 和 source->spawn 边", () => {
  ...
  assert.deepEqual(next.edges, [
    { source: "线索发现", target: "疑点辩论工厂", triggerOn: "transfer", messageMode: "last" },
  ]);
});
