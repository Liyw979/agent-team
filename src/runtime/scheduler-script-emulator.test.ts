...
type MatchesExpectedTransitionBaseInput =
  Omit<MatchesExpectedTransitionCoreInput, "routing">;
...
test("scheduler script emulator 纯函数在 finished 状态下不会再为下一条 sender 提供额外候选筛选", () => {
  ...
  assert.deepEqual(preferred.map((item) => item.result.routing.trigger), ["<continue>", "<complete>"]);
});
