...
test("extractDslAgentsFromTopology 会把单节点 Build 拓扑解析为非可空 Agent 列表", () => {
...
test("extractDslAgentsFromTopology 不会把未显式配置 writable 的 Build 视为默认可写", () => {
  const resolved = extractDslAgentsFromTopology({
    ...
    nodeRecords: [
      ...
      { id: "BA", kind: "agent", templateName: "BA", prompt: "你是 BA。", writable: false, initialMessageRouting: { mode: "inherit" } },
    ],
  });
