...
test("scheduler script harness 支持断言 execute_batch、waiting 与 finished 调度决策", async () => {
  const topology = createTopology({
    projectId: "scheduler-script-decision-assertions",
    downstream: {
      BA: { Build: "association" },
      Build: {
        CodeReview: "association",
...
test("脚本模式支持用户首条直接 @reviewer 并沿 needs_revision 回路继续推进", async () => {
  const topology = createTopology({
    ...
    downstream: {
      安全负责人: { 漏洞分析人员: "needs_revision" },
      漏洞分析人员: { 安全负责人: "needs_revision" },
    },
  });
  const script = [
...
test("通用调度脚本模式支持非 Build 的实现者反复修复 reviewer 意见", async () => {
  const topology = createTopology({
    ...
    downstream: {
      ...
      UnitTest: { Implementer: "needs_revision" },
    },
  });
...
test("当前调度不支持在 needs_revision 对弈中由发起方直接跳到裁决", async () => {
  const topology = createTopology({
    ...
    downstream: {
      初筛: { 反方: "association" },
      正方: { 反方: "needs_revision", 裁决: "approved" },
      反方: { 正方: "needs_revision", 裁决: "approved" },
      裁决: { 初筛: "approved" },
    },
  });
...
test("脚本模式支持通用 reviewer 在回流超限后转给 approved 下游裁决", async () => {
  const topology = createTopology({
    projectId: "generic-script-loop-limit-approved-escalation",
    downstream: {
...
test("脚本模式会读取 needs_revision 边上的 maxRevisionRounds 覆盖值", async () => {
  const topology = createTopology({
    projectId: "generic-script-loop-limit-override",
...
test("当前日志场景在 Build 重新派发给 CodeReview 和 TaskReview 后脚本调度仍处于等待 reviewer", async () => {
  const topology = createTopology({
    projectId: "generic-script-current-log-finished-state",
    nodes: ["BA", "Build", "UnitTest", "TaskReview", "CodeReview"],
    edges: [
      createEdge("BA", "Build", "association"),
      createEdge("Build", "UnitTest", "association"),
