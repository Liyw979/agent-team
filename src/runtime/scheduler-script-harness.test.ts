...
test("scheduler script harness 支持断言 execute_batch 与 finished 调度决策", async () => {
  const topology = createTopology({
    downstream: {
      BA: { Build: "transfer" },
      Build: {
  ...
  await assertSchedulerScript({
    ...
    expectedDecisions: [
      ...
      { type: "finished", finishReason: "wait_pending_reviewers" },
...
test("当前调度不支持在 action_required 对弈中由发起方直接跳到裁决", async () => {
  const topology = createTopology({
    downstream: {
      线索发现: { 漏洞挑战: "transfer" },
      漏洞论证: { 漏洞挑战: "continue", 讨论总结: "complete" },
      漏洞挑战: { 漏洞论证: "continue", 讨论总结: "complete" },
      讨论总结: { 线索发现: "complete" },
    },
  });
  ...
  const script = [
    ...
    "线索发现: 发现第 1 个可疑点：上传文件名可能被直接拼进目标路径。 @漏洞挑战",
    "漏洞挑战: 目前证据不够，缺少从入口到落盘路径的完整调用链。 @漏洞论证",
    "漏洞论证: 我已补齐入口、过滤逻辑与落盘点，请漏洞挑战重新判断。 @漏洞挑战",
    "漏洞挑战: 当前证据链已经闭环，直接提交讨论总结。 @讨论总结",
  ];
...
test("漏洞挖掘团队可以通过 action_required 对弈并在讨论总结后回到线索发现继续下一轮", async () => {
  const topology = createTopology({
    downstream: {
      线索发现: { 漏洞挑战: "transfer" },
      漏洞论证: { 漏洞挑战: "continue", 讨论总结: "complete" },
  ...
  const script = [
    ...
    "线索发现: 发现第 1 个可疑点：上传文件名可能被直接拼进目标路径。 @漏洞挑战",
    "漏洞挑战: 目前证据不够，缺少从入口到落盘路径的完整调用链。 @漏洞论证",
    "漏洞论证: 我已补齐入口、过滤逻辑与落盘点，当前证据链已经闭环，可以进入讨论总结。 @讨论总结",
    "讨论总结: 该点成立为漏洞，回到线索发现继续寻找下一个可疑点。 @线索发现",
    "线索发现: 发现第 2 个可疑点：内部调试接口似乎缺少鉴权。 @漏洞挑战",
    "漏洞挑战: 这次证据不足，更像误报，交给讨论总结。 @讨论总结",
    "讨论总结: 该点暂不成立，继续回到线索发现寻找下一处。 @线索发现",
  ];
...
test("漏洞挖掘团队脚本支持论证挑战对弈超限后自动转给讨论总结", async () => {
  ...
  const pro1 = "漏洞论证-spawn1";
  const con1 = "漏洞挑战-spawn1";
  const summary1 = "讨论总结-spawn1";

  const script = [
    "user: @线索发现 请持续挖掘当前代码中的可疑漏洞点。",
    `线索发现: 发现第 1 个可疑点：上传文件名可能被直接拼进目标路径。 @${con1}`,
    ...
    `${summary1}: 该点是否成立已可直接判断，回到线索发现继续下一处。 @线索发现`,
  ];

  await assertSchedulerScript({ topology, script });
});

test("漏洞挖掘团队脚本里讨论总结完成后会回到线索发现继续挖掘", () => {
  const topology = compileTeamDsl(readBuiltinTopology("vulnerability-team.topology.json")).topology;
  const script = [
    "user: @线索发现 请持续挖掘当前代码中的可疑漏洞点。",
    "线索发现: 发现第 1 个可疑点：上传文件名可能被直接拼进目标路径。 @漏洞挑战-1",
    "漏洞挑战-1: 当前材料更像误报，可以进入讨论总结。 @讨论总结-1",
    "讨论总结-1: 通过 @线索发现",
  ];
  const state = createGraphTaskState({
    taskId: "task-vulnerability-summary-returns-to-lead-discovery",
    topology,
  });
  ...
  assert.equal(script[1], "线索发现: 发现第 1 个可疑点：上传文件名可能被直接拼进目标路径。 @漏洞挑战-1");
  ...
  const afterCon = applyAgentResultToGraphState(afterTriage.state, {
    ...
    agentContextContent: "当前材料更像误报，可以进入讨论总结。",
    opinion: null,
    allowDirectFallbackWhenNoBatch: false,
    signalDone: false,
  });
  assert.equal(script[2], "漏洞挑战-1: 当前材料更像误报，可以进入讨论总结。 @讨论总结-1");
  ...
  assert.equal(script[3], "讨论总结-1: 通过 @线索发现");
...
test("漏洞挖掘团队脚本支持按不同 spawn 实例区分 runtime agent id", async () => {
  ...
  const pro1 = "漏洞论证-1";
  const con1 = "漏洞挑战-1";
  const summary1 = "讨论总结-1";
  const con2 = "漏洞挑战-2";
  const summary2 = "讨论总结-2";
  ...
  const script = [
    ...
    `${summary1}: 该点成立为漏洞，输出正式漏洞报告。 @线索发现`,
    `线索发现: 发现第 2 个可疑点：内部调试接口似乎缺少鉴权。 @${con2}`,
    `${con2}: 当前材料更像误报，可以进入裁决。 @${summary2}`,
    `${summary2}: 该点暂不成立。 @线索发现`,
  ];

  await assertSchedulerScript({ topology, script });
});

test("漏洞挖掘团队脚本支持用 漏洞挑战-spawn1 / 漏洞论证-spawn1 这类短别名区分不同 spawn 实例", async () => {
  const topology = compileTeamDsl(readBuiltinTopology("vulnerability-team.topology.json")).topology;

  const pro1 = "漏洞论证-spawn1";
  const con1 = "漏洞挑战-spawn1";
  const summary1 = "讨论总结-spawn1";
  const con2 = "漏洞挑战-spawn2";
  const summary2 = "讨论总结-spawn2";
