...
test("并发 reviewer 中单条回流链路超限时，不应提前打断其他 reviewer", () => {
  const topology: TopologyRecord = {
    ...
    edges: [
      ...
      { source: "Build", target: "TaskReview", triggerOn: "association", messageMode: "last" },
      { source: "Build", target: "CodeReview", triggerOn: "association", messageMode: "last" },
      { source: "UnitTest", target: "Build", triggerOn: "needs_revision", maxRevisionRounds: 1, messageMode: "last" },
      { source: "UnitTest", target: "Judge", triggerOn: "approved", messageMode: "last" },
      { source: "TaskReview", target: "Build", triggerOn: "needs_revision", messageMode: "last" },
      { source: "CodeReview", target: "Build", triggerOn: "needs_revision", messageMode: "last" },
    ],
  };
...
test("回流超限时，如果 reviewer 正文已包含最终结论提示，不应再重复追加一遍", () => {
  const topology: TopologyRecord = {
    ...
    edges: [
      { source: "Build", target: "反方-1", triggerOn: "association", messageMode: "last" },
      { source: "反方-1", target: "Build", triggerOn: "needs_revision", messageMode: "last", maxRevisionRounds: 1 },
      { source: "反方-1", target: "裁决总结-1", triggerOn: "approved", messageMode: "last" },
    ],
  };
  let state = createGraphTaskState({
