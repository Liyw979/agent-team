...
function link(
  from: string,
  to: string,
  trigger_type: "association" | "approved" | "needs_revision",
...
function createDevelopmentGraphDsl() {
  return {
    ...
    links: [
      link("BA", "Build", "association", "last"),
      link("Build", "CodeReview", "association", "last"),
      link("Build", "UnitTest", "association", "last"),
      link("Build", "TaskReview", "association", "last"),
      link("CodeReview", "Build", "needs_revision", "last"),
      link("UnitTest", "Build", "needs_revision", "last"),
      link("TaskReview", "Build", "needs_revision", "last"),
    ],
  };
}
...
test("compileTeamDsl 支持把递归式图 DSL 编译成 agents + topology", () => {
  const compiled = compileTeamDsl({
    ...
    links: [
      link("BA", "Build", "association", "last"),
      link("Build", "SecurityResearcher", "association", "last"),
      link("SecurityResearcher", "Build", "needs_revision", "last"),
    ],
  });
  ...
  assert.deepEqual(compiled.topology.edges, [
    ...
    { source: "SecurityResearcher", target: "Build", triggerOn: "needs_revision", messageMode: "last" },
  ]);
    ...
    nodes: [
      ...
      spawnNode(
        "疑点辩论",
        {
          ...
          links: [
            link("正方", "反方", "needs_revision", "last"),
            link("反方", "正方", "needs_revision", "last"),
            link("正方", "裁决总结", "approved", "last"),
            link("反方", "裁决总结", "approved", "last"),
          ],
        },
      ),
    ],
    links: [
      link("初筛", "疑点辩论", "association", "last"),
      link("疑点辩论", "初筛", "association", "last"),
    ],
  });
...
test("compileTeamDsl 支持在 links 上显式声明边级消息传递策略", () => {
  const compiled = compileTeamDsl({
    ...
    links: [
      link("初筛", "疑点辩论", "association", "all"),
      link("疑点辩论", "裁决总结", "association", "last"),
      link("裁决总结", "初筛", "association", "none"),
    ],
  });

  assert.deepEqual(compiled.topology.edges, [
    { source: "初筛", target: "疑点辩论", triggerOn: "association", messageMode: "all" },
    { source: "疑点辩论", target: "裁决总结", triggerOn: "association", messageMode: "last" },
    { source: "裁决总结", target: "初筛", triggerOn: "association", messageMode: "none" },
  ]);
});
    ...
    matchesAppliedTeamDslTopology(
      {
        ...
        edges: [{ source: "Build", target: "BA", triggerOn: "association", messageMode: "last" }],
      },
      compiled,
    ),
    false,
