...
test("本项目内提供开发团队与漏洞挖掘团队拓扑文件", () => {
  ...
  const vulnerabilityTeamFile = path.join(BUILTIN_TOPOLOGY_DIR, "vulnerability-team.topology.json5");
...
test("内置团队拓扑 JSON5 节点使用 id 字段而不是 name 字段", () => {
  const topologyFiles = [
    ...
    "vulnerability-team.topology.json5",
  ];
...
test("漏洞挖掘团队默认使用论证与挑战多轮对弈，而不是固定四个辩手串行两轮", () => {
  const vulnerabilityTeamTopology = readBuiltinTopology("vulnerability-team.topology.json5");
  const compiled = compileBuiltinTopology("vulnerability-team.topology.json5");
