...
test("漏洞挖掘团队默认使用正反双方多轮对弈，而不是固定四个辩手串行两轮", () => {
  ...
  assert.equal(
    vulnerabilityTeamTopology.links.some((link: BuiltinLink) =>
      link.from === "初筛" && link.to === "__end__" && link.trigger_type === "complete"),
    true,
  );
});

test("漏洞挖掘团队里初筛是条件流转：有 finding 走 continue，没有 finding 走 complete -> END", () => {
  const vulnerabilityTeamTopology = readBuiltinTopology("vulnerability-team.topology.json");
  const compiled = compileTeamDsl(vulnerabilityTeamTopology);

  assert.equal(
    compiled.topology.edges.some((edge) =>
      edge.source === "初筛" && edge.target === "疑点辩论" && edge.triggerOn === "continue"),
    true,
  );
  assert.deepEqual(compiled.topology.langgraph?.end, {
    id: "__end__",
...
test("漏洞挖掘团队的初筛 prompt 明确要求回复开头先输出标签再输出正文", () => {
  const vulnerabilityTeamTopology = readBuiltinTopology("vulnerability-team.topology.json");
  const initialScreeningNode = findBuiltinAgentNode(vulnerabilityTeamTopology.nodes, "初筛");
