...
test("开发团队拓扑包含 CodeReview 审查回路", () => {
  ...
  assert.equal(
    developmentTeamTopology.links.some(
      (link: BuiltinLink) => link.from === "Build" && link.to === "CodeReview" && link.trigger_type === "association",
    ),
    true,
  );
  assert.equal(
    developmentTeamTopology.links.some(
      (link: BuiltinLink) => link.from === "CodeReview" && link.to === "Build" && link.trigger_type === "needs_revision",
    ),
    true,
  );
});
