...
import { runSchedulerScriptDrived } from "./scheduler-script-emulator";
...
test("漏洞挖掘团队可以通过 action_required 对弈并在讨论总结后回到线索发现继续下一轮", async () => {
  ...
  const script = [
    ...
    "漏洞挑战-1: 目前证据不够，缺少从入口到落盘路径的完整调用链。 @漏洞论证-1",
    "漏洞论证-1: 我已补齐入口、过滤逻辑与落盘点，当前证据链已经闭环，可以进入讨论总结。 @讨论总结-1",
    "讨论总结-1: 该点成立为漏洞，回到线索发现继续寻找下一个可疑点。 @线索发现",
    "线索发现: 发现第 2 个可疑点：内部调试接口似乎缺少鉴权。 @漏洞挑战-2",
    "漏洞挑战-2: 这次证据不足，更像误报，还需要进一步补证。 @漏洞论证-2",
    "漏洞论证-2: 已补齐调试路由注册与环境开关信息，当前更像误报，可以进入讨论总结。 @讨论总结-2",
    "讨论总结-2: 该点暂不成立，继续回到线索发现寻找下一处。 @线索发现",
...
test("漏洞挖掘团队脚本支持用 漏洞挑战-spawn1 / 漏洞论证-spawn1 这类短别名区分不同 spawn 实例", async () => {
  ...
  await assert.rejects(
    () => runSchedulerScriptDrived({ topology, script }),
    /不存在的 Agent|漏洞挑战-spawn1|漏洞论证-spawn1/u,
  );
});
...
test("真实日志里重复 reviewer 回复并在整批失败后再次全量派发，不应通过脚本校验", async () => {
  ...
  await assert.rejects(
    () => runSchedulerScriptDrived({ topology, script }),
    /脚本包含 \[Build\]|当前步骤模拟值为 \[Build\]|不是当前批次 .* 等待中的 reviewer|sender 不在当前 execute_batch 目标里|@ 目标与预期不一致|无法继续推进脚本|缺少 Build 第 2 轮回复|无法唯一推断/u,
  );
});
