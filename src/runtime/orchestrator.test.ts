...
test("漏洞团队里漏洞挑战返回 complete 后会继续派发到讨论总结，而不是后台中断", async () => {
  const userDataPath = createTempDir();
  const projectPath = createTempDir();
  const orchestrator = createTestOrchestrator({
    userDataPath,
...
test("漏洞团队里讨论总结以 transfer + none 回到线索发现时，会下发 continue 指令继续找线索", async () => {
  ...
  typed.opencodeRunner.run = async ({ agent, content }) => {
    ...
    if (agent === "漏洞挑战") {
      return buildCompletedExecutionResult({
        agent,
        finalMessage: "<complete>当前材料已经足够，进入讨论总结。</complete>",
        messageId: `message:${agent}:${count}`,
...
test("漏洞团队 spawn runtime agent 尚未落库时，getTaskSnapshot 不会把任务提前判 finished", async () => {
  ...
  const revisionRequestMessage = typed.store.listMessages(projectPath, task.task.id).findLast(
    (message) => message.kind === "continue-request" && message.sender === "线索发现",
  );
  const runtimeAgentIdFromRevision = revisionRequestMessage
    ? getMessageTargetAgentIds(revisionRequestMessage)[0] ?? null
    : null;

  assert.equal(typeof runtimeAgentIdFromRevision, "string");
  assert.equal(runtimeAgentIdFromRevision?.startsWith("漏洞挑战-"), true);
  assert.equal(taskAgentIdsDuringDispatchWindow.includes(runtimeAgentIdFromRevision!), false);
...
test("message_type = last-all 的转发会复用运行时生成的群聊语义卡片，不会重复拼接 continue-request", async () => {
  ...
  assert.equal(
    ...
    [
      "[Build] Build 首轮实现完成。",
      `[TaskReview] ${finalBody}\n\n${reviewBody}`,
    ].join("\n\n"),
  );
});

test("审查 Agent 未返回合法标签时默认按 continue 处理", () => {
  const orchestrator = createTestOrchestrator({
    userDataPath: createTempDir(),
    enableEventStream: false,
  });
  ...
  ).parseReview("这是普通审查正文，标签写错了。\n\n<invalid>请继续补充实现依据。</invalid>", true);
...
test("审查 Agent 未返回合法标签时会沿 continue 边继续回流", async () => {
  const userDataPath = createTempDir();
  const projectPath = createTempDir();
  const orchestrator = createTestOrchestrator({
    userDataPath,
  ...
  const taskReviewFinal = snapshot.messages.findLast(
    (message) => message.sender === "TaskReview" && message.kind === "agent-final",
  );
  assert.notEqual(taskReviewFinal, undefined);
  assert.equal(taskReviewFinal?.kind, "agent-final");
  if (!taskReviewFinal || taskReviewFinal.kind !== "agent-final") {
    assert.fail("缺少 TaskReview 的最终消息");
  }
  assert.equal(taskReviewFinal.reviewDecision, "continue");
  assert.equal(taskReviewFinal.content, "当前证据链还不完整，请继续补充实现依据。\n\n<chalenge>请继续补充实现依据。</chalenge>");
