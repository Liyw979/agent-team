...
test("漏洞团队里反方返回 complete 后会继续派发到裁决总结，而不是后台中断", async () => {
  const userDataPath = createTempDir();
  const projectPath = createTempDir();
  const orchestrator = createTestOrchestrator({
    userDataPath,
  ...
  await waitForValue(
    async () => {
      const dispatchMessage = typed.store.listMessages(projectPath, task.task.id).findLast(
        (message) => message.kind === "continue-request" && message.sender === "初筛",
      );
...
test("漏洞团队 spawn runtime agent 尚未落库时，getTaskSnapshot 不会把任务提前判 finished", async () => {
  ...
  assert.equal(runtimeAgentIdFromRevision?.startsWith("反方-"), true);
