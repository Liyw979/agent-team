...
function requireSingleTriggeredAgentFinalMessage(
  ...
  routing: { kind: "triggered"; trigger: string };
} {
  const message = requireSingleMessage(
    messages.filter((item) => item.kind === "agent-final"),
    description,
  );
  if (message.routing.kind !== "triggered") {
    assert.fail(`${description}：最终消息不是 triggered`);
  }
  return message as Extract<MessageRecord, { kind: "agent-final" }> & {
    routing: { kind: "triggered"; trigger: string };
  };
}
...
test("单节点任务进入 finished 时不会因为缺少 workspace cwd 而在后台崩溃", async () => {
  ...
  await orchestrator.submitTask("@BA 请分析当前问题");
...
test("任务本轮 finished 后再次 @Agent 时会回到 running 并在同一 Task 内完成下一轮", async () => {
  ...
  await orchestrator.submitTask("@BA 请先完成第一轮");
  ...
  const reopened = await orchestrator.submitTask("@BA 请继续第二轮");
...
test("漏洞团队里误报论证先返回触发回流的 label、漏洞论证回应后才会继续派发到讨论总结", async () => {
  ...
  await orchestrator.submitTask("@线索发现 请分析这个漏洞线索");
...
test("漏洞团队里讨论总结以 transfer + none 回到线索发现时，会下发对应的继续查找请求", async () => {
  ...
  await orchestrator.submitTask("@线索发现 请持续挖掘当前代码中的可疑漏洞点。");
...
test("漏洞团队第二轮 finding 已经派发到 误报论证-2 时，UI 仍能看到线索发现的第二轮消息", async () => {
  ...
  await orchestrator.submitTask("@线索发现 请持续挖掘当前代码中的可疑漏洞点，直到没有新 finding 为止。");
...
test("Agent 间传递不再携带 [Initial Task]，只保留来源 Agent 段落", async () => {
  ...
  await orchestrator.submitTask("@BA 请实现 add 方法，并补充验证说明。");
...
test("agent 声明 initialMessage 后，下游实际 prompt 会保留默认转发并额外注入来源段落", async () => {
  ...
  await orchestrator.submitTask("@线索发现 请继续分析这个漏洞线索。");
...
test("initialMessage 已包含当前触发 agent 时，最终 prompt 不会重复注入同一个来源段落", async () => {
  ...
  await orchestrator.submitTask("@A 请继续推进。");
...
test("当前 Project 缺少 Build Agent 时，默认会从 start node 开始，显式 @Build 仍会被拒绝", async () => {
  ...
  const submittedTask = await orchestrator.submitTask("@BA 请先整理需求。");
  ...
  const defaultSubmittedTask = await orchestrator.submitTask("请先整理需求。");
  ...
  await assert.rejects(async () => orchestrator.submitTask("@Build 请先实现需求。"), /@Build 不可用/u);
});
...
test("单 decisionAgent 返回 <continue> 后会按 trigger 回流给 Build", async () => {
  ...
  await orchestrator.submitTask("@BA 请实现 add 方法，并准备判定修复。");
...
test("首个 <continue> 触发上游重派发后，会按上游默认边重新派发全部下游 decisionAgent", async () => {
  ...
  await orchestrator.submitTask("@Build 请完成这个需求。");
...
test("Task 启动后仍允许重新 applyTeamDsl，让 task ui 的 --file 继续以 .yaml 为准", async () => {
  const userDataPath = createTempDir();
  const projectPath = createTempDir();
  const orchestrator = new TestOrchestrator({
    cwd: projectPath,
...
test("单 Agent 且没有下游时，任务结束后仍保留该 Agent 的最终聊天消息", async () => {
  ...
  await orchestrator.submitTask("@BA 请输出一句验证成功。");
...
---`;
  ...
  const submittedTask = await orchestrator.submitTask("@BA 请输出完整结果。");
...
---`;
  ...
  await orchestrator.submitTask("@漏洞论证 请输出完整结果。");
...
test("自定义 trigger 会按精确标签触发约定下游", async () => {
  ...
  await orchestrator.submitTask("@漏洞论证 请继续当前争议点的论证。");
...
test("custom-only trigger 返回未声明的示例 label 时会直接失败，不会误派发下游", async () => {
  ...
  if (!argumentMessage || argumentMessage.kind !== "agent-final" || argumentMessage.routing.kind !== "invalid") {
    assert.fail("缺少漏洞论证的最终消息");
  }
  assert.equal(argumentMessage.routing.kind, "invalid");
...
test("自定义结束 trigger 可以直接命中 __end__", async () => {
  ...
  await orchestrator.submitTask("@漏洞论证 请完成本轮论证。");
  ...
  if (!argumentMessage || argumentMessage.kind !== "agent-final" || argumentMessage.routing.kind !== "triggered") {
    assert.fail("缺少漏洞论证的最终消息");
  }
...
test("判定 Agent 未返回合法标签时必须直接判 invalid 并终止任务", async () => {
  ...
  assert.equal(taskDecisionFinal.routing.kind, "invalid");
...
test("最大连续回流达到上限后，聊天页面会直接展示明确失败原因", async () => {
  ...
  await orchestrator.submitTask("@Build 请完成需求并通过 UnitTest");
  orchestrator.opencodeClient.submitMessage = async (_sessionId, { agent }) => {
    if (agent === "Build") {
      buildRunCount += 1;
      return buildCompletedExecutionResult({
