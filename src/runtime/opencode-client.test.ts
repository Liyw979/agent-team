...
test("request 失败时会写入 task 级失败日志", async () => {
  ...
  const [latestRecord] = readTaskEventRecords(
    userDataPath,
    "task-request-failed",
    "opencode.request_failed",
...
test("submitMessage 请求级失败由 submit 重试接管且每次重试前先 abort", async () => {
  ...
  const stdoutRequestFailedRecords = stdoutRecords.filter((record) => record["event"] === "opencode.request_failed");
  assert.equal(stdoutRequestFailedRecords.length, 1);
  const [stdoutRequestFailedRecord] = stdoutRequestFailedRecords as [Record<string, unknown>];
  assert.equal(stdoutRequestFailedRecord["method"], "POST");
  assert.equal(stdoutRequestFailedRecord["url"], "http://127.0.0.1:43127/session/session-1/message");
  assert.equal(stdoutRequestFailedRecord["responseBody"], "server error");
  assert.deepEqual(
    stdoutRecords
  ...
  assert.equal(firstRequestFailure["method"], "POST");
  assert.equal(firstRequestFailure["url"], "http://127.0.0.1:43127/session/session-1/message");
  assert.equal(firstRequestFailure["responseBody"], "server error");
...
test("submitMessage 在空 assistant final 后会重新发消息", async () => {
  ...
  const { client } = createClient();
  const submittedContents: string[] = [];
  const emptyAssistantEnvelope = JSON.stringify({
    info: {
      id: "msg-empty",
      role: "assistant",
      time: {
  ...
  let postCount = 0;
  const { result, stdout } = await captureStdout(() => withMockedFetch((async (...args: Parameters<typeof fetch>) => {
    const pathname = new URL(String(args[0])).pathname;
    if (pathname === "/session/session-1/abort") {
      return new Response("", { status: 200 });
    }
    ...
    submittedContents.push(body.parts[0].text);
    if (postCount === 1) {
      return new Response(emptyAssistantEnvelope, {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
  ...
  assert.equal(record["responseBody"], emptyAssistantEnvelope);
  const stdoutRecords = stdout.trim().split("\n").map((line) => JSON.parse(line) as Record<string, unknown>);
  const retriedRecords = stdoutRecords.filter((entry) => entry["event"] === "opencode.submit_message_retried");
  const [stdoutRecord] = retriedRecords as [Record<string, unknown>];
  assert.equal(stdoutRecord["responseBody"], emptyAssistantEnvelope);
});
...
test("submitMessage 在 final 缺少 trigger 后会重新发完整回复要求", async () => {
  ...
  assert.equal(record["responseBody"], "<456> 非法判定");
  const stdoutRecords = stdout.trim().split("\n").map((line) => JSON.parse(line) as Record<string, unknown>);
  const retriedRecords = stdoutRecords.filter((entry) => entry["event"] === "opencode.submit_message_retried");
  const [stdoutRecord] = retriedRecords as [Record<string, unknown>];
  assert.equal(stdoutRecord["responseBody"], "<456> 非法判定");
});
...
test("submitMessage 在 OpenCode error 消息后会记录错误并重新发消息", async () => {
  ...
  assert.equal(record["responseBody"], "模型执行失败");
  const stdoutRecords = stdout.trim().split("\n").map((line) => JSON.parse(line) as Record<string, unknown>);
  const retriedRecords = stdoutRecords.filter((entry) => entry["event"] === "opencode.submit_message_retried");
  const [stdoutRecord] = retriedRecords as [Record<string, unknown>];
  assert.equal(stdoutRecord["responseBody"], "模型执行失败");
});
