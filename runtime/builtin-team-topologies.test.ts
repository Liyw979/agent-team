...
test("开发团队拓扑包含 CodeReview 审查回路", () => {
  ...
  const nodeNames = developmentTeamTopology.nodes.map((node) => node.name);
  ...
  assert.equal(
    developmentTeamTopology.links.some(
      (link) => link[0] === "Build" && link[1] === "CodeReview" && link[2] === "association",
    ),
    true,
  );
  assert.equal(
    developmentTeamTopology.links.some(
      (link) => link[0] === "CodeReview" && link[1] === "Build" && link[2] === "needs_revision",
    ),
    true,
  );
});
...
test("漏洞挖掘团队默认使用正反双方多轮对弈，而不是固定四个辩手串行两轮", () => {
  ...
  const debateNode = vulnerabilityTeamTopology.nodes.find((node) => node.name === "疑点辩论");
  const summaryNode = debateNode?.type === "spawn"
    ? debateNode.graph.nodes.find((node) => node.name === "裁决总结")
    : undefined;

  assert.notEqual(spawnRule, undefined);
  assert.equal(spawnRule?.name, "疑点辩论");
  assert.equal((debateNode as { itemsFrom?: string } | undefined)?.itemsFrom, undefined);
  assert.equal(spawnRule?.itemsFrom, "items");
  assert.equal(spawnRule?.reportToTemplateName, "初筛");
  assert.equal(spawnRule?.reportToTriggerOn, "association");
...
test("漏洞团队的裁决总结 prompt 约束漏洞输出报告、误报不额外输出，并按 association 回到初筛", () => {
  const vulnerabilityTeamTopology = readBuiltinTopology("vulnerability-team.topology.json");
  const debateNode = vulnerabilityTeamTopology.nodes.find((node) => node.name === "疑点辩论");
  const summaryNode = debateNode?.type === "spawn"
    ? debateNode.graph.nodes.find((node) => node.name === "裁决总结")
...
test("漏洞挖掘团队的初筛 prompt 要求每次仅返回一个可疑漏洞点", () => {
  const vulnerabilityTeamTopology = readBuiltinTopology("vulnerability-team.topology.json");
  const initialScreeningNode = vulnerabilityTeamTopology.nodes.find(
    (node) => node.name === "初筛",
  );
...
test("漏洞挖掘团队的结论类 agent prompt 要求先阅读代码再给结论", () => {
  ...
  const proNode = debateAgents.find((node) => node.name === "正方");
  const conNode = debateAgents.find((node) => node.name === "反方");
  const summaryNode = debateAgents.find((node) => node.name === "裁决总结");
...
test("内置拓扑里的单个 agent prompt 不显式提及其他 agent", () => {
  ...
  const developmentAgentNames = developmentTeamTopology.nodes.map((node) => node.name);
  ...
  const vulnerabilityRootAgentNames = vulnerabilityTeamTopology.nodes.map((node) => node.name);
  for (const node of vulnerabilityTeamTopology.nodes) {
    ...
    const childAgentNames = node.graph.nodes.map((child) => child.name);
