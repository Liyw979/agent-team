...
test("合并回应消息时只保留一份回应与一份 mention", () => {
  const summary =
    `${REVIEW_CHALLENGE_LABEL}暂无进一步结论，因为尚未完成润色工作。请先完成需求润色，然后再回应实现是否成立。`
    + REVIEW_CHALLENGE_END_LABEL;
...
test("revision-request 单独展示时也会移除孤立的结束标签后再追加 mention", () => {
  const merged = mergeTaskChatMessages([
    createMessage({
      id: "revision-request",
      content: `${REVIEW_CHALLENGE_END_LABEL}请补充实现依据。\n\n@Build`,
      meta: {
        kind: "revision-request",
        targetAgentId: "Build",
      },
  ...
  const merged = mergeTaskChatMessages([
    createMessage({
      ...
      meta: {
        kind: "agent-final",
        reviewDecision: "needs_revision",
        finalMessage: `审视不通过。\n\n${REVIEW_CHALLENGE_LABEL}请补充实现依据。`,
      },
    }),
  ]);
  ...
  const merged = mergeTaskChatMessages([
    createMessage({
      ...
      meta: {
        kind: "agent-final",
        reviewDecision: "needs_revision",
        finalMessage: `审视不通过。\n\n${REVIEW_CHALLENGE_END_LABEL}请补充实现依据。`,
      },
    }),
  ]);
...
test("spawn 实例消息在聊天展示中优先使用 senderDisplayName，保留模板名和实例 id", () => {
  const merged = mergeTaskChatMessages([
    createMessage({
      ...
      sender: "entry#spawn-rule:TaskReview:spawn-rule:TaskReview-0001",
      content: "这版已经达到可交付标准。",
      meta: {
        kind: "agent-final",
