...
test("脚本模式支持用户首条直接 @reviewer 并沿 review_fail 回路继续推进", async () => {
  const topology = createTopology({
    ...
    nodes: ["安全负责人", "漏洞分析人员"],
    edges: [
      createEdge("安全负责人", "漏洞分析人员", "review_fail"),
      createEdge("漏洞分析人员", "安全负责人", "review_fail"),
    ],
  });
  const script = [
...
test("当前调度不支持在 review_fail 对弈中由发起方直接跳到裁决", async () => {
  const topology = createTopology({
    ...
    edges: [
      ...
      createEdge("正方", "反方", "review_fail"),
      createEdge("反方", "正方", "review_fail"),
      createEdge("正方", "裁决", "review_pass"),
      createEdge("反方", "裁决", "review_pass"),
      createEdge("裁决", "初筛", "review_pass"),
    ],
  });
