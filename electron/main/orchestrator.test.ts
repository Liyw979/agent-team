...
test("单 reviewer 审查失败后会把 challenge 回流给 Build", async () => {
  const userDataPath = createTempDir();
  const projectPath = createTempDir();
  const orchestrator = createTestOrchestrator({
    userDataPath,
  ...
  typed.opencodeRunner.run = async ({ agent }) => {
    ...
    if (agent === "UnitTest") {
      ...
      if (unitTestRunCount === 1) {
        return completedResponse(
          agent,
          unitTestRunCount,
          "UnitTest 第 1 轮未通过。\n\n<challenge>请修复 UnitTest 第 1 轮问题。</challenge>",
        );
      }
    ...
    if (agent === "TaskReview") {
      ...
      if (taskReviewRunCount === 1) {
        return completedResponse(
          agent,
          taskReviewRunCount,
          "TaskReview 第 1 轮未通过。\n\n<challenge>请修复 TaskReview 第 1 轮问题。</challenge>",
        );
      }
    ...
    if (codeReviewRunCount === 1) {
      return completedResponse(
        agent,
        codeReviewRunCount,
        "CodeReview 第 1 轮未通过。\n\n<challenge>请修复 CodeReview 第 1 轮问题。</challenge>",
      );
    }
...
test("审查 Agent 返回 challenge 后会在其余 reviewer 收齐后回流到 Build", async () => {
  const userDataPath = createTempDir();
  const projectPath = createTempDir();
  const orchestrator = createTestOrchestrator({
    userDataPath,
    ...
    if (agent === "UnitTest") {
      ...
      return unitTestRunCount === 1
        ? completedResponse(
            agent,
            unitTestRunCount,
            "UnitTest 第 1 轮未通过。\n\n<challenge> 请修复第 1 轮单测问题。</challenge>",
          )
  ...
  const displayContent = (
    orchestrator as unknown as {
      createDisplayContent: (
        parsedReview: {
          cleanContent: string;
          decision: "pass" | "needs_revision" | "invalid";
          opinion: string | null;
...
test("审视不通过且只返回 challenge 标签时，群聊展示会去掉标签", () => {
  const orchestrator = createTestOrchestrator({
    userDataPath: createTempDir(),
    enableEventStream: false,
  });
  ...
  const displayContent = (
  ...
  ).createDisplayContent(
    {
      ...
      opinion: "请继续补充实现依据。",
      rawDecisionBlock: "<challenge> 请继续补充实现依据。",
      validationError: null,
    },
    null,
  );
...
test("审视 Agent 执行中止时不会伪造成整改意见", async () => {
  ...
  assert.equal(
    snapshot.messages.some(
      (message) => message.content.includes("<challenge> Aborted"),
    ),
    false,
  );
  assert.equal(
  ...
  typed.opencodeRunner.run = async ({ agent }) => {
    ...
    if (agent === "UnitTest") {
      ...
      return {
        ...
        finalMessage:
          unitTestRunCount === 1
            ? "UnitTest 未通过。\n\n<challenge>请修复 UnitTest。</challenge>"
            : "UnitTest 通过。\n\n<agree>同意当前结果。</agree>",
        fallbackMessage: null,
    ...
    return {
      ...
      finalMessage:
        taskReviewRunCount === 1
          ? "TaskReview 未通过。\n\n<challenge>请修复 TaskReview。</challenge>"
          : "TaskReview 通过。\n\n<agree>同意当前结果。</agree>",
      fallbackMessage: null,
