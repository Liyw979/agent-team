...
- 拓扑图中的 Agent 节点顺序是稳定的：可在拓扑配置中指定一个“最左起点” Agent，默认优先取 `BA`
...
- 运行中的 Agent 在 Zellij 中会按运行态重排 pane：单个运行中 Agent 优先占左侧半屏，多个运行中 Agent 优先排到左上区域
- 全新 Task 首次初始化、且当前还没有托管 pane 时，Zellij 会优先按最多三列的 tiled grid 创建初始 pane 布局
- 运行中的 Agent 会通过 OpenCode HTTP session 消息接口轮询实时工具调用与摘要，并显示在拓扑图节点内
- CLI 默认使用当前目录作为 project cwd
- CLI 既支持当前 Agent 短名，也兼容历史 `-Agent` 别名，便于兼容旧数据
