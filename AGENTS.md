...
- GUI 中点击 Agent 支持直接编辑并保存当前 Agent 的名称与 prompt；“内置 Agent 模板”仍是可选项，不会自动写入当前 Project，可先单独编辑模板 prompt 再按需写入为 Agent，且模板修改只影响当前 Project，不影响新项目默认值；一旦当前 Project 出现 Task 启动记录，Agent 与内置模板配置都会被锁定，不允许继续修改
