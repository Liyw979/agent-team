...
- `TaskReview` 默认内置模板使用“任务交付审视”文案：重点从用户价值、业务目标、功能实现与验收证据判断结果是否可交付，不审代码风格、架构选型或局部实现细节；只有这些实现问题已经直接影响功能成立、验收结论或交付风险时，才作为交付问题指出。
- `CodeReview` 默认内置模板使用“代码审查”文案：必须主动阅读实际代码实现，直接检查核心代码路径、边界处理、状态流转、异常路径、冗余实现与可读性，不能只复述其他 Agent 对代码的结论。
