...
- 已验证可成功生成 Windows 可执行目录包的命令为 `npx electron-builder --win dir --x64 --config.win.signAndEditExecutable=false`；产物主程序位于 `dist/win-unpacked/agentflow.exe`
...
- GUI 中点击 Agent 支持直接编辑并保存当前 Agent 的名称与 prompt；“内置 Agent 模板”仍是可选项，不会自动写入当前 Project，可先单独编辑模板 prompt 再按需写入为 Agent，且模板修改只影响当前 Project，不影响新项目默认值；一旦当前 Project 出现任务运行记录（进入任务驱动阶段），仅允许更新已有 Agent 的 prompt，Agent 名称修改、新增与删除会被锁定，但模板 prompt 仍可继续维护
