...
- 只允许用户自定义 prompt；当前 Project 可以不设置可写 Agent，也可以从已写入当前 Project 的 Agent 中指定 1 个作为可写 Agent；同一 Project 中最多只能有一个
...
- GUI 中点击 Agent 支持直接编辑并保存当前 Agent 的名称、prompt 与可写状态；“内置 Agent 模板”仍是可选项，不会自动写入当前 Project，可先单独编辑可编辑模板的 prompt 再按需写入为 Agent，且模板修改只影响当前 Project，不影响新项目默认值。Build 也作为默认模板提供，但它使用 OpenCode 自带 prompt，不支持在这里修改 prompt，仅支持按需写入当前 Project、删除已写入项，以及在写入时按需设为当前 Project 的可写 Agent；同一 Project 中最多只能有一个可写 Agent，也允许不设置；一旦当前 Project 出现 Task 启动记录，Agent 与内置模板配置都会被锁定，不允许继续修改
