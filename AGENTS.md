...
- Agent 一旦给出需要继续响应的 `回应：...` 尾段，系统会把当前 Task 直接收口为“不通过”；失败链路不再单独派发固定 Agent
...
- Agent 自动触发下游 Agent 时，会封装 `[User Message]` 与带真实 Agent 名称的 `[<AgentName> Message]` 结构化段落；同时会在 `[Requeirement]` 段附带当前 Project Git Diff 的精简摘要，帮助下游 Agent 快速感知最新改动
